...
import com.owl.kafka.consumer.listener.BatchAcknowledgeMessageListener;
import com.owl.kafka.consumer.listener.MessageListener;
import com.owl.kafka.consumer.service.BatchAcknowledgeMessageListenerService;
...
public class DefaultKafkaConsumerImpl<K, V> implements Runnable, com.owl.kafka.consumer.KafkaConsumer<K, V> {
    ...
    public DefaultKafkaConsumerImpl(com.owl.kafka.consumer.ConsumerConfig configs) {
        ...
        if(!this.configs.isUseProxy()){
            ...
            if(configs.get("partition.assignment.strategy") == null){
                configs.put("partition.assignment.strategy", "CheckTopicStickyAssignor");
            }
        ...
        if (start.compareAndSet(false, true)) {
            if(useProxy){
                ...
                if(isAssignTopicPartition){
                    Collection<com.owl.kafka.consumer.TopicPartition> assignTopicPartitions = configs.getTopicPartitions();
                    ArrayList<TopicPartition> topicPartitions = new ArrayList<>(assignTopicPartitions.size());
                    for(com.owl.kafka.consumer.TopicPartition topicPartition : assignTopicPartitions){
